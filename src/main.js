...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Helper to generate short links via OuO API
  const getShortLink = (url) => {
    if (!url || url === '#') return '#';
    if (url.startsWith('http')) {
      return `http://ouo.io/qs/53Bk9bUR?s=${encodeURIComponent(url)}`;
    }
    return url;
  };

  // Helper function to get provider name from URL
  const getProviderName = (url) => {
    if (!url) return 'Enlace';
    const lowUrl = url.toLowerCase();
    if (lowUrl.includes('mega.nz') || lowUrl.includes('megaup')) return 'Mega';
    if (lowUrl.includes('1fichier')) return '1Fichier';
    if (lowUrl.includes('drive.google')) return 'Google Drive';
    if (lowUrl.includes('mediafire')) return 'MediaFire';
    if (lowUrl.includes('gofile')) return 'Gofile';
    if (lowUrl.includes('send.now')) return 'Send.now';
    if (lowUrl.includes('datanodes')) return 'Datanodes';
    if (lowUrl.includes('qiwi')) return 'Qiwi';
    if (lowUrl.includes('pixeldrain')) return 'Pixeldrain';
  ...
  // NEWS SYSTEM
  // ============================================================
  let aiNews = latestNewsData.articles || [];
  ...
  const renderGames = () => {
    ...
    // Toggle news sections visibility depending on search query and category
    const newsTicker = document.querySelector('.news-flash');
    const shouldHideNews = (searchQuery.trim().length > 0) || (currentCategory !== 'ALL' && !newsForceVisible);

    if (shouldHideNews) {
      if (featuredGrid) featuredGrid.style.display = 'none';
      if (newsTicker) newsTicker.style.display = 'none';
      if (toggleNewsBtn && searchQuery.trim().length === 0) {
        toggleNewsBtn.style.display = 'inline-block';
        toggleNewsBtn.textContent = 'Mostrar Noticias';
      } else if (toggleNewsBtn) {
        toggleNewsBtn.style.display = 'none';
      }
    } else {
      if (featuredGrid) featuredGrid.style.display = 'grid';
      if (newsTicker) newsTicker.style.display = 'flex';
      if (toggleNewsBtn) {
        if (currentCategory !== 'ALL' && newsForceVisible) {
          toggleNewsBtn.style.display = 'inline-block';
          toggleNewsBtn.textContent = 'Ocultar Noticias';
        } else {
          toggleNewsBtn.style.display = 'none';
        }
      }
...
{"genre":{"en":"...","es":"..."},"releaseDate":{"en":"...","es":"..."},"publisher":{"en":"...","es":"..."},"developer":{"en":"...","es":"..."},"size":{"en":"...","es":"..."},"description":{"en":"80-100 word synopsis of the game's story and gameplay.","es":"Sinopsis de 80-100 palabras de la historia y jugabilidad del juego."}}`;
  ...
  const renderGamePage = async (game) => {
        ...
        <div class="game-main-content">
          ...
          <div class="download-section-box">
            ...
            <div class="download-list">
              ${(() => {
                const linksData = (game.links && game.links.length > 0 ? game.links : (game.link && game.link !== '#' ? [game.link] : []));
                const providerTotals = {};
                linksData.forEach(url => {
                  const p = getProviderName(url);
                  providerTotals[p] = (providerTotals[p] || 0) + 1;
                });
                const providerCounters = {};
                
                return linksData.map((url, idx) => {
                  const provider = getProviderName(url);
                  providerCounters[provider] = (providerCounters[provider] || 0) + 1;
                  const isMultiPart = providerTotals[provider] > 1;
                  const badgeText = isMultiPart ? `PARTE ${providerCounters[provider]}` : `MIRROR ${idx + 1}`;
              ...
              <div class="download-link-row">
                <div class="download-provider">
                  ...
                  <span>${provider}</span>
                  <span class="download-format-badge">${badgeText}</span>
                </div>
                <a href="${getShortLink(url)}" target="_blank" class="download-action-btn">${t.downloadBtn}</a>
              </div>
              `;
                }).join('');
              })()}
    ...
    const finalTrailer = details.trailer || game.trailer;
    let trailerHtml = '';
    if (finalTrailer) {
      const ytMatch = finalTrailer.match(/(?:youtu.be\/|youtube.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))([\w-]{11})/);
        ...
        trailerHtml = `
          <div class="game-trailer" style="margin-top: 20px;">
            ...
            <div class="video-container" style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; border-radius: 8px;">
              <iframe src="https://www.youtube.com/embed/${ytMatch[1]}" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
            </div>
          </div>
        `;
      }
    ...
    // Update download mirror buttons and sizes dynamically
    const finalLinksData = finalLinks; // already handles overrides
    const providerTotals = {};
    finalLinksData.forEach(url => {
      const p = getProviderName(url);
      providerTotals[p] = (providerTotals[p] || 0) + 1;
    });
    const providerCounters = {};
    ...
    mirrorLinks.forEach((btn, idx) => {
      const url = finalLinksData[idx] || game.link;
      btn.href = getShortLink(url);
      ...
      if (mirrorBadges[idx]) {
        const badgeText = isMultiPart ? `PARTE ${providerCounters[provider]}` : `MIRROR ${idx + 1}`;
        mirrorBadges[idx].textContent = `${badgeText} (${size})`;
      }
    });
  };
